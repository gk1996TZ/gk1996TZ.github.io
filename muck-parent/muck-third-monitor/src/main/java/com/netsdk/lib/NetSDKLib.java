...
public interface NetSDKLib extends Library {
    ...
    public static final int EVENT_IVSS_FACEATTRIBUTE            = 0x00000235;       // IVSS人脸检测事件 (暂未有具体事件)
    public static final int EVENT_IVSS_FACEANALYSIS             = 0x00000236;       // IVSS人脸分析事件 (暂未有具体事件)
    public static final int EVENT_IVSS_FACECOMPARE              = 0x00000237;       // IVSS人脸识别事件 (暂未有具体事件)
    ...
    public static final int NET_COMMON_STRING_8                 = 8;    // 通用字符串长度32
    ...
    public static class EM_SCENE_TYPE extends Structure
    {
    	...
    	public static final int EM_SCENE_FACEATTRIBUTE = 30;		// "FaceAttribute" IVSS人脸检测
    	public static final int EM_SCENE_FACECOMPARE = 31;			// "FaceCompare" IVSS人脸识别
    	public static final int EM_SCENE_CROWD_DISTRI_MAP = 32;		// "CrowdDistriMap" 人群分布图
    }
    ...
    // 信息
        ...
        public CFG_TIME_SECTION_WEEK_DAY[]  stuTimeSectionArr = (CFG_TIME_SECTION_WEEK_DAY[])new CFG_TIME_SECTION_WEEK_DAY().toArray(WEEK_DAY_NUM);// 事件响应时间段
    }
    ...
    // 事件类型EVENT_IVSS_FACEANALYSIS(IVSS人脸分析事件) 对应的规则配置
    public static class CFG_IVSSFACEANALYSIS_INFO extends Structure
    {
    	public byte[] 						szRuleName = new byte[MAX_NAME_LEN];							// 规则名称,不同规则不能重名
        public byte 						bRuleEnable;						    						// 规则使能,bool类型，取值0或1
        public byte[] 						bReserved = new byte[3];										// 保留字
    ...
    public static class FACERECOGNITION_PERSON_INFOEX extends Structure 
    {
    	...
    	public int							nFeatureState;										// 是否从人脸图片计算了特征值, 0-未知, 1-建模失败, 2-有可用的特征值, 3-正在计算特征值, 4-已建模但需要重新建模		
    	public byte[]                       byReserved = new byte[2048];   						// 保留字节
    } 
 ...
 public static class DEV_EVENT_TRAFFIC_PASSNOTINORDER_INFO extends Structure
 {
    ...
    {
        public int                   dwSize;
        public Pointer    			 pstuFiles;                  // 文件信息数组, 用户分配内存, 对应 NET_REMOTE_FILE_INFO[],大小为sizeof(DH_REMOTE_FILE_INFO)*nMaxFileCount
   ...
   public static class NET_MONITORWALL_OUTPUT extends Structure
	...
	public static class NET_IN_REALPLAY_BY_DATA_TYPE extends Structure
	{
	    ...
	    public LLong           			dwUser;                 // 用户数据, Win32时，要改成NativeLong类型
	...
	public static class AV_CFG_MonitorWallBlock extends Structure
	{
		...
		public CFG_TIME_SECTION_EX[]		stuTimeSectionArr = (CFG_TIME_SECTION_EX[])new CFG_TIME_SECTION_EX().toArray(WEEK_DAY_NUM);	// 开关机时间
		...
		public AV_CFG_MonitorWallBlock() {
			...
			for(int i = 0; i < AV_CFG_Max_TV_In_Block; i++) {
				stuTVs[i] = new AV_CFG_MonitorWallTVOut();
			}
		}
	}
	
	public static class CFG_TIME_SECTION_EX extends Structure
    {
		public CFG_TIME_SECTION[]		stuTimeSection = (CFG_TIME_SECTION[])new CFG_TIME_SECTION().toArray(MAX_REC_TSECT);	// 开关机时间	
    }
	...
	public static class CFG_TIME_SECTION_WEEK_DAY extends Structure
	{
		public CFG_TIME_SECTION[]	 			stuTimeSection = (CFG_TIME_SECTION[])new CFG_TIME_SECTION().toArray(MAX_REC_TSECT_EX);	 // 事件响应时间段
	}
	
	// 通用设备信息
	public static class NET_COMMON_DEVICE_INFO extends Structure
	{
		public byte[]		        szDeviceID = new byte[NET_DEV_ID_LEN_EX]; 	// 设备ID
		public byte[] 			    byReserved = new byte[512];                 // 保留字节
	}
	...
	public static class NET_CB_ATTACH_DEVICE_STATE extends Structure
	{
		...
		public Pointer			pstuDeviceInfo;		                // 设备列表, NET_COMMON_DEVICE_INFO[]
		public byte[] 			byReserved = new byte[512];         // 保留字节
	}
	...
	// CLIENT_AddDevice 添加设备入参结构
	public static class NET_IN_ADD_DEVICE extends Structure
	{
	    public int                           dwSize;
		public int							 nCount;																								//URL个数
	    public DEVICE_URL[]		 			 szUrlsArr = (DEVICE_URL[])new DEVICE_URL().toArray(MAX_COMMON_STRING_16); 		//URL组
																																					// 格式：协议名://用户名:密码@IP:端口 端口不填则用协议默认端口
		public NET_IN_ADD_DEVICE() {
	       this.dwSize = this.size();
	    }
	}
	...
	// CLIENT_AddDevice 添加设备出参结构
	public static class NET_OUT_ADD_DEVICE extends Structure
	{
	    public int           				dwSize;
		public int							nTaskID;		//任务ID
		
		public NET_OUT_ADD_DEVICE() {
	       this.dwSize = this.size();
	    }
	}
	...
	public static class NET_IN_GET_ADD_DEVICE_LIST_INFO extends Structure
	{
	    ...
	    public int					nTaskID;										// 任务ID
	    public int					nCount;											// 设备个数
	    public Pointer              pnIndex;           								// 设备序号列表(NET_IN_ADD_DEVICE中szUrls的序号，从0开始), int[]
	...
	public static class DEVICE_INDEX extends Structure {
		public int                nIndex;           								// 设备序号
	}
	...
	public static class NET_OUT_GET_ADD_DEVICE_LIST_INFO extends Structure
	{
	    public int               		dwSize;
	    public int						nCount;											// 设备个数
	    public Pointer            		pstuDeviceInfo;           						// 设备信息列表, NET_GET_ADD_DEVICE_INFO[]
																						// 内存由用户维护, 大小为nCount 个 NET_GET_ADD_DEVICE_INFO
	...
	public static class NET_IN_GET_DEVICE_LIST_INFO extends Structure
	{
	    public int               	dwSize;
	    public int					nCount;											// 设备个数
	    public Pointer            	pstuDeviceInfo;           						// 设备信息列表, NET_COMMON_DEVICE_INFO[]
	...
	public static class NET_GET_DEVICE_INFO extends Structure
	{
		...
		public DEVICE_MAC[]	 		 szMacsArr = (DEVICE_MAC[])new DEVICE_MAC().toArray(NET_COMMON_STRING_8);	// 设备mac地址组
	...
	public static class NET_OUT_GET_DEVICE_LIST_INFO extends Structure
	{
	    public int               	dwSize;
	    public int					nCount;											// 设备个数
	    public Pointer            	pstuDeviceInfo;           						// 设备信息列表, NET_GET_DEVICE_INFO[]
																					// 内存由用户维护, 大小为nCount 个 NET_GET_DEVICE_INFO
	...
	public static class NET_IN_SET_CONNECT_CHANNEL extends Structure
	{
	    ...
	    public Pointer            	pnChannels;           							// 通道列表, int[]
																					// 内存由用户维护, 大小为nCount 个 int
	...
	public static class NET_OUT_GET_CHANNEL_INFO extends Structure
	{
	    public int					dwSize;
		public int					nCount;					// 设备个数
		public Pointer				pstuChannelInfo;		// 通道信息列表, NET_GET_CHANNEL_INFO[]
															// 内存由用户维护, 大小为nCount 个 NET_GET_CHANNEL_INFO
    ...
    public interface fServiceCallBack extends StdCallCallback {
    	public int invoke(NativeLong lHandle, String pIp, int wPort, NativeLong lCommand, Pointer pParam, int dwParamLen, NativeLong dwUserData);
    }
    ...
    public interface fDeviceStateCallBack extends StdCallCallback {
    	public void invoke(NativeLong lAttachHandle, NET_CB_ATTACH_DEVICE_STATE pstDeviceState, NativeLong dwUser);
    }

    // 注册添加设备的回调函数原型
    public interface fAddDeviceCallBack extends StdCallCallback {
    	public void invoke(NativeLong lAttachHandle, NET_CB_ATTACH_ADD_DEVICE pstAddDevice, NativeLong dwUser);
    } 

    // 定义监测点报警信息回调函数原型
    public interface fSCADAAlarmAttachInfoCallBack extends StdCallCallback {
    	public void invoke(NativeLong lAttachHandle, NET_SCADA_NOTIFY_POINT_ALARM_INFO_LIST pInfo, int nBufLen, NativeLong dwUser);
    }
    ...
    public boolean  CLIENT_DetectFace(NativeLong lLoginID, NET_IN_DETECT_FACE pstInParam, NET_OUT_DETECT_FACE pstOutParam, int nWaitTime);
    ...
    public boolean  CLIENT_OperateFaceRecognitionGroup(NativeLong lLoginID, NET_IN_OPERATE_FACERECONGNITION_GROUP pstInParam, NET_OUT_OPERATE_FACERECONGNITION_GROUP pstOutParam, int nWaitTime);
    
    // 查询人脸识别人员组信息
    // pstInParam指向NET_IN_FIND_GROUP_INFO类型的指针
    // pstOutParam指向NET_OUT_FIND_GROUP_INFO类型的指针
    public boolean  CLIENT_FindGroupInfo(NativeLong NativeLong, NET_IN_FIND_GROUP_INFO pstInParam, NET_OUT_FIND_GROUP_INFO pstOutParam, int nWaitTime);
    
    // 获取布控在视频通道的组信息,pstInParam与pstOutParam内存由用户申请释放
    public boolean CLIENT_GetGroupInfoForChannel(NativeLong lLoginID, NET_IN_GET_GROUPINFO_FOR_CHANNEL pstInParam, NET_OUT_GET_GROUPINFO_FOR_CHANNEL pstOutParam, int nWaitTime);

    // 布控通道人员组信息
    // pstInParam指向NET_IN_SET_GROUPINFO_FOR_CHANNEL类型的指针
    // pstOutParam指向NET_OUT_SET_GROUPINFO_FOR_CHANNEL类型的指针
    public boolean CLIENT_SetGroupInfoForChannel(NativeLong lLoginID, NET_IN_SET_GROUPINFO_FOR_CHANNEL pstInParam, NET_OUT_SET_GROUPINFO_FOR_CHANNEL pstOutParam, int nWaitTime);
    
    // 以人脸库的角度进行布控, pstInParam与pstOutParam内存由用户申请释放
    public boolean CLIENT_FaceRecognitionPutDisposition(NativeLong lLoginID, NET_IN_FACE_RECOGNITION_PUT_DISPOSITION_INFO pstInParam, NET_OUT_FACE_RECOGNITION_PUT_DISPOSITION_INFO pstOutParam, int nWaitTime);

    // 以人脸库的角度进行撤控, pstInParam与pstOutParam内存由用户申请释放
    public boolean CLIENT_FaceRecognitionDelDisposition(NativeLong lLoginID, NET_IN_FACE_RECOGNITION_DEL_DISPOSITION_INFO pstInParam, NET_OUT_FACE_RECOGNITION_DEL_DISPOSITION_INFO pstOutParam, int nWaitTime);

    // 订阅人脸查询状态
    // pstInParam指向NET_IN_FACE_FIND_STATE类型的指针
    // pstOutParam指向NET_OUT_FACE_FIND_STATE类型的指针
    public NativeLong CLIENT_AttachFaceFindState(NativeLong lLoginID, NET_IN_FACE_FIND_STATE pstInParam, NET_OUT_FACE_FIND_STATE pstOutParam, int nWaitTime);
    ...
    public boolean CLIENT_DownloadRemoteFile(NativeLong lLoginID, NET_IN_DOWNLOAD_REMOTE_FILE pInParam, NET_OUT_DOWNLOAD_REMOTE_FILE pOutParam, int nWaitTime);
    ...
    // 抓图同步接口,将图片数据直接返回给用户
    public boolean CLIENT_SnapPictureToFile(NativeLong lLoginID, NET_IN_SNAP_PIC_TO_FILE_PARAM pInParam, NET_OUT_SNAP_PIC_TO_FILE_PARAM pOutParam, int nWaitTime);
    ...
    public NativeLong CLIENT_DownloadByTimeEx(NativeLong lLoginID, int nChannelId, int nRecordFileType, 
								             NET_TIME tmStart, NET_TIME tmEnd, String sSavedFileName, 
								             StdCallCallback cbTimeDownLoadPos, NativeLong dwUserData, 
    ...
    ******************************************************************************/
    public NativeLong CLIENT_DownloadByTimeEx2(NativeLong lLoginID, int nChannelId, int nRecordFileType, 
                                               NET_TIME tmStart, NET_TIME tmEnd, String sSavedFileName, 
                                               StdCallCallback cbTimeDownLoadPos, NativeLong dwUserData, 
                                               StdCallCallback fDownLoadDataCallBack, NativeLong dwDataUser, 
	...
	//------------------------用户管理-----------------------
  	...
  	public boolean CLIENT_QueryUserInfoEx(NativeLong lLoginID, USER_MANAGE_INFO_EX info, int waittime);
    ...
    // 同步文件上传, 只适用于小文件
  	...
  	public boolean CLIENT_ListRemoteFile(NativeLong lLoginID, NET_IN_LIST_REMOTE_FILE pInParam, NET_OUT_LIST_REMOTE_FILE pOutParam, int nWaitTime);
  	
  	// 删除文件或目录,pInParam与pOutParam内存由用户申请释放
  	public boolean CLIENT_RemoveRemoteFiles(NativeLong lLoginID, NET_IN_REMOVE_REMOTE_FILES pInParam, NET_OUT_REMOVE_REMOTE_FILES pOutParam, int nWaitTime);
    ...
    // 查询/设置分割模式(pstuSplitInfo内存由用户申请释放)
  	...
  	public boolean CLIENT_OpenSplitWindow(NativeLong lLoginID, NET_IN_SPLIT_OPEN_WINDOW pInParam, NET_OUT_SPLIT_OPEN_WINDOW pOutParam, int nWaitTime);
  	public boolean CLIENT_CloseSplitWindow(NativeLong lLoginID, NET_IN_SPLIT_CLOSE_WINDOW pInParam, NET_OUT_SPLIT_CLOSE_WINDOW pOutParam, int nWaitTime);

  	// 获取当前显示的窗口信息(pInparam, pOutParam内存由用户申请释放)
  	public boolean CLIENT_GetSplitWindowsInfo(NativeLong lLoginID, NET_IN_SPLIT_GET_WINDOWS pInParam, NET_OUT_SPLIT_GET_WINDOWS pOutParam, int nWaitTime);
...
//	public NativeLong CLIENT_AttachBurnState(NativeLong lLoginID, NET_IN_ATTACH_STATE pstInParam, NET_OUT_ATTACH_STATE pstOutParam, int nWaitTime);
	...
	// 指定回调数据格式  开始回放,  数据回调函数 fDownLoadDataCallBack 中得到的码流类型为 emDataType 所指定的类型
  	public NativeLong CLIENT_PlayBackByDataType(NativeLong lLoginID, NET_IN_PLAYBACK_BY_DATA_TYPE pstInParam, NET_OUT_PLAYBACK_BY_DATA_TYPE pstOutParam, int dwWaitTime);

	// 指定码流类型 开始下载, 下载得到的文件和数据回调函数 fDownLoadDataCallBack 中得到的码流类型均为 emDataType 所指定的类型
  	public NativeLong CLIENT_DownloadByDataType(NativeLong lLoginID, NET_IN_DOWNLOAD_BY_DATA_TYPE pstInParam, NET_OUT_DOWNLOAD_BY_DATA_TYPE pstOutParam, int dwWaitTime);
   ...
   public boolean CLIENT_SubcribeGPSTempHumidity(NativeLong lLoginID, int bStart,  int InterTime, Pointer Reserved);
    ...
    **********************************************************************************/
   // 设置诱导屏配置信息接口
   public boolean CLIENT_SetGuideScreenCfg(NativeLong lLoginID, NET_IN_SET_GUIDESCREEN_CFG pInParam, NET_OUT_SET_GUIDESCREEN_CFG pstOutPqram, int nWaitTime);
  
   // 添加一个节目信息到诱导屏
   public boolean CLIENT_AddOneProgramme(NativeLong lLoginID, NET_IN_ADD_ONE_PROGRAMME pInParam, NET_OUT_ADD_ONE_PROGRAMME pOutParam, int nWaitTime);

   // 通过节目ID 修改节目
   public boolean CLIENT_ModifyOneProgrammeByID(NativeLong lLoginID, NET_IN_MODIFY_ONE_PROGRAMME pInParam, NET_OUT_MODIFY_ONE_PROGRAMME pOutParam, int nWaitTime);

   // 批量删除节目信息
   public boolean CLIENT_DelMultiProgrammesById(NativeLong lLoginID, NET_IN_DEL_PROGRAMMES pInParam, NET_OUT_DEL_PROGRAMMES pOutParam, int nWaitTime);
   
   // 增加一个即时节目计划
   public boolean CLIENT_AddOneImmediProgrammePlan(NativeLong lLoginID, NET_IN_ADD_IMME_PROGRAMMEPLAN pInParam, NET_OUT_ADD_PROGRAMMEPLAN pOutParam, int nWaitTime);

   // 修改一个即时节目计划
   public boolean CLIENT_ModifyOneImmediProgrammePlan(NativeLong lLoginID, NET_IN_MODIFY_IMME_PROGRAMMEPLAN pInParam, NET_OUT_MODIFY_IMME_PROGRAMMEPLAN pOutParam, int nWaitTime);
   
   // 增加一个定时节目计划
   public boolean CLIENT_AddOneTimerProgrammePlan(NativeLong lLoginID, NET_IN_ADD_TIMER_PROGRAMMEPLAN pInParam, NET_OUT_ADD_PROGRAMMEPLAN pOutParam, int nWaitTime);
   
   // 修改一个定时节目计划
   public boolean CLIENT_ModifyOneTimerProgrammePlan(NativeLong lLoginID, NET_IN_MODIFY_TIMER_PROGRAMMEPLAN pInParam, NET_OUT_MODIFY_TIMER_PROGRAMMEPLAN pOutParam, int nWaitTime);

   // 删除多个节目计划
   public boolean CLIENT_DelMultiProgrammePlans(NativeLong lLoginID, NET_IN_DEL_PROGRAMMEPLANS pInParam, NET_OUT_DEL_PROGRAMMEPLANS pOutParam, int nWaitTime);
   
   // 通过诱导屏ID 获取诱导屏配置信息
   public boolean CLIENT_GetOneGuideScreenCfgById(NativeLong lLoginID, NET_IN_GET_GUIDESCREEN_CFG_BYID pInParam, NET_OUT_GET_GUIDESCREEN_CFG_BYID pOutParam, int nWaitTime);

   // 获取所有诱导屏配置信息
   public boolean CLIENT_GetAllGuideScreenCfg(NativeLong lLoginID, NET_IN_GET_ALL_GUIDESCREEN_CFG pInParam, NET_OUT_GET_ALL_GUIDESCREEN_CFG pOutParam, int nWaitTime);

   // 通过节目ID 获取节目信息
   public boolean CLIENT_GetOneProgrammeById(NativeLong lLoginID, NET_IN_GET_PROGRAMME_BYID pInParam, NET_OUT_GET_PROGRAMME_BYID pOutParam, int nWaitTime);

   // 获取所有节目信息
   public boolean CLIENT_GetAllProgrammes(NativeLong lLoginID, NET_IN_GET_ALL_PROGRAMMES pInParam, NET_OUT_GET_ALL_PROGRAMMES pOutParam, int nWaitTime);

   // 获取所有节目的简要信息
   public boolean CLIENT_GetAllBrieflyProgrammes(NativeLong lLoginID, NET_IN_GET_ALL_BRIEFLYPROGRAMMES pInParam, NET_OUT_GET_ALL_BRIEFLYPROGRAMMES pOutParam, int nWaitTime);

   // 获取所有节目计划信息
   public boolean CLIENT_GetAllProgrammePlans(NativeLong lLoginID, NET_IN_GET_ALL_PROGRAMMEPLANS pInParam, NET_OUT_GET_ALL_PROGRAMMEPLANS pOutParam, int nWaitTime);

   // 通过节目计划ID 获取节目计划
   public boolean CLIENT_GetOneProgrammePlanByID(NativeLong lLoginID, NET_IN_GET_PROGRAMMEPLAN_BYID pInParam, NET_OUT_GET_PROGRAMMEPLAN_BYID pOutParam, int nWaitTime);

   // 设置光带状态信息
   public boolean CLIENT_SetGuideScreenGDStatus(NativeLong lLoginID, NET_IN_SET_GD_STATUS pInParam, NET_OUT_SET_GD_STATUS pOutParam, int nWaitTime);
   
   /***********************************************************************************
	*						                  播放盒与广告机的节目操作接口					      *
    **********************************************************************************/
  // 获取播放盒上全部节目信息
  public boolean CLIENT_GetAllProgramOnPlayBox(NativeLong lLoginID, NET_IN_GET_ALL_PLAYBOX_PROGRAM pInParam, NET_OUT_GET_ALL_PLAYBOX_PROGRAM pOutParam, int nWaitTime);

  // 通过programme ID 获取播放盒上对应的节目信息
  public boolean CLIENT_GetOneProgramByIdOnPlayBox(NativeLong lLoginID, NET_IN_GET_PLAYBOX_PROGRAM_BYID pInParam, NET_OUT_GET_PLAYBOX_PROGRAM_BYID pOutParam, int nWaitTime);
  
  // 在播放盒上添加一个节目
  public boolean CLIENT_AddOneProgramToPlayBox(NativeLong lLoginID, NET_IN_ADD_ONE_PLAYBOX_PRAGROM pInParam, NET_OUT_ADD_ONE_PLAYBOX_PRAGROM pOutParam, int nWaitTime);

  // 在播放盒上修改指定ID的节目信息
  public boolean CLIENT_ModifyProgramOnPlayBoxById(NativeLong lLoginID, NET_IN_MODIFY_PLAYBOX_PROGRAM_BYID pInParam, NET_OUT_MODIFY_PLAYBOX_PROGRAM_BYID pOutParam, int nWaitTime); 


  
  // 获取配置信息(szOutBuffer内存由用户申请释放, 具体见枚举类型 NET_EM_CFG_OPERATE_TYPE 说明)
  public boolean CLIENT_GetConfig(NativeLong lLoginID, int emCfgOpType, int nChannelID, Pointer szOutBuffer, int dwOutBufferSize, int waittime, Pointer reserve);

  // 设置配置信息(szInBuffer内存由用户申请释放, 具体见枚举类型 NET_EM_CFG_OPERATE_TYPE 说明)
  public boolean CLIENT_SetConfig(NativeLong lLoginID, int emCfgOpType, int nChannelID, Pointer szInBuffer, int dwInBufferSize, int waittime, IntByReference restart, Pointer reserve);

  // 显示私有数据，例如规则框，规则框报警，移动侦测等       lPlayHandle:播放句柄      bTrue=1 打开, bTrue= 0 关闭
  public boolean CLIENT_RenderPrivateData(NativeLong lPlayHandle, int bTrue);
  
  // 按设备信息添加显示源,pInParam与pOutParam内存由用户申请释放
  public boolean CLIENT_MatrixAddCamerasByDevice(NativeLong lLoginID, NET_IN_ADD_LOGIC_BYDEVICE_CAMERA pInParam, NET_OUT_ADD_LOGIC_BYDEVICE_CAMERA pOutParam, int nWaitTime);

  // 订阅监测点位报警信息,pInParam与pOutParam内存由用户申请释放
  public NativeLong CLIENT_SCADAAlarmAttachInfo(NativeLong lLoginID, NET_IN_SCADA_ALARM_ATTACH_INFO pInParam, NET_OUT_SCADA_ALARM_ATTACH_INFO pOutParam, int nWaitTime);
	
  // 取消订阅监测点位报警信息
  public boolean CLIENT_SCADAAlarmDetachInfo(NativeLong lAttachHandle);
  
  /***********************************************************************************
	*						           IVSS设备添加相关接口				             *
    **********************************************************************************/  
  // 注册设备状态回调
  public NativeLong CLIENT_AttachDeviceState(NativeLong lLoginID, NET_IN_ATTACH_DEVICE_STATE pInParam, NET_OUT_ATTACH_DEVICE_STATE pOutParam, int nWaitTime);

  // 注销设备状态回调
  public boolean CLIENT_DetachDeviceState(NativeLong lAttachHandle);

  // 添加设备
  public boolean CLIENT_AddDevice(NativeLong lLoginID, NET_IN_ADD_DEVICE pInParam, NET_OUT_ADD_DEVICE pOutParam, int nWaitTime);
	
  // 注册添加设备回调
  public NativeLong CLIENT_AttachAddDevice(NativeLong lLoginID, NET_IN_ATTACH_ADD_DEVICE pInParam, NET_OUT_ATTACH_ADD_DEVICE pOutParam, int nWaitTime);
	
  // 注销添加设备回调
  public boolean CLIENT_DetachAddDevice(NativeLong lAttachHandle);
	
  // 获取添加中的设备状态
  public boolean CLIENT_GetAddDeviceInfo(NativeLong lLoginID, NET_IN_GET_ADD_DEVICE_LIST_INFO pInParam, NET_OUT_GET_ADD_DEVICE_LIST_INFO pOutParam, int nWaitTime);
	
  // 获取已添加的设备状态
  public boolean CLIENT_GetDeviceInfo(NativeLong lLoginID, NET_IN_GET_DEVICE_LIST_INFO pInParam, NET_OUT_GET_DEVICE_LIST_INFO pOutParam, int nWaitTime);
	
  // 设置连接通道
  public boolean CLIENT_SetConnectChannel(NativeLong lLoginID, NET_IN_SET_CONNECT_CHANNEL pInParam, NET_OUT_SET_CONNECT_CHANNEL pOutParam, int nWaitTime);
	
  // 获取设备通道信息
  public boolean CLIENT_GetChannelInfo(NativeLong lLoginID, NET_IN_GET_CHANNEL_INFO pInParam, NET_OUT_GET_CHANNEL_INFO pOutParam, int nWaitTime);
	
  // 删除设备
  public boolean CLIENT_RemoveDevice(NativeLong lLoginID, NET_IN_REMOVE_DEVICE pInParam, NET_OUT_REMOVE_DEVICE pOutParam, int nWaitTime);
  
  // 中止添加设备任务
  public boolean CLIENT_CancelAddDeviceTask(NativeLong lLoginID, NET_IN_CANCEL_ADD_TASK pInParam, NET_OUT_CANCEL_ADD_TASK pOutParam, int nWaitTime);

  // 确认添加设备任务
  public boolean CLIENT_ConfirmAddDeviceTask(NativeLong lLoginID, NET_IN_CONFIRM_ADD_TASK pInParam, NET_OUT_CONFIRM_ADD_TASK pOutParam, int nWaitTime);
}
