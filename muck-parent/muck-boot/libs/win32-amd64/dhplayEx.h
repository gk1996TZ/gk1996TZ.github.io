...
typedef enum 
{
	...
	ENCODE_AUDIO_G726_40,           /*40kbps,以下32/24/16*/
	ENCODE_AUDIO_G726_32,           /*分别表示比特率的不同*/
	ENCODE_AUDIO_G726_24,           /*相对于8k采样率的*/
	ENCODE_AUDIO_G726_16,           /*情况下*/
...
typedef enum GetSetType
{
	...
	RENDER_STEREO_EYE_MOVE_RIGHT,                    //往左移动
...
typedef struct 
{
	int nProcessType;      //处理类型, 见#DECODE_DATA_PROCESS_TYPE枚举类型
...
#define IVSINFOTYPE_OVERLAY_PIC     11 /*叠加图片帧, 辅助帧(0x12)*/ 
#define IVSINFOTYPE_OSD_INFO        12 /*叠加OSD信息，辅助帧(0x11)*/
#define IVSINFOTYPE_GPS_INFO        13 /*GPS辅助帧*/
#define IVSINFOTYPE_TAGGING_INFO    14 /* 景物点信息标注帧,辅助帧(0x13) */
#define PSIVS						240
...
typedef void (CALLMETHOD *fIVSInfoCallbackFunc)(char* buf, LONG type, LONG len, LONG reallen, void* pReserved, void* pUserData);
...
//		 pData, 数据指针
//		 nDataLen, 数据长度
//		 nOffset, 从起始数据偏移位置, avi格式会回写,外部需按照此偏移来写文件, -1表示此字段无效
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
/************************************************************************/
/* 函数: PLAY_SetMultiSensorCallBack
   描述: 设置多目视频源坐标信息回调
   参数: nPort,通道号
   参数: pSensor:多目坐标指针, 共mSensor*nSensor大小
		 mSensor: m元
		 nSensor: n元
		 nPicWidth: 码流宽
		 nPicHeight:码流高
...
//------------------------------------------------------------------------
// 函数:PLAY_SetOSDInfoCallBack
// 描述: osd数据回调
// 参数: nPort,通道号
//		 fOSDInfoCallBackFun,数据回调指针
//			nPort,通道号
//			pBuf,数据指针
//			nSize,数据长度
//			pUserData,用户自定义数据
...
//------------------------------------------------------------------------
// 函数: PLAY_SetDisplayRegionEx
// 描述: 设置或增加显示区域,可以做局部放大显示或显示窗口部分区域.
// 参数: nPort,通道号
//		 nRegionNum,显示区域序号,0~(MAX_DISPLAY_WND-1),如果为0,则将设置的区域显示在主窗口中.
//		 pSrcRect,视频画面局部显示区域,范围为[0, 视频宽][0, 视频高]
//		 pDstRect,部分窗口显示区域,客户区坐标
//		 hDestWnd,显示窗口句柄
...
//------------------------------------------------------------------------
// 函数: PLAY_SetDecodeDataProcessCallBack
// 描述: 解码后数据回调处理，结果存入相应缓存。
// 参数: nPort,通道号
//        nProcessType,数据处理类型, 见#DECODE_DATA_PROCESS_TYPE
//		 cbDecDataProcess,解码回调处理函数指针,不能为NULL.回调函数参数含义如下:
//			nPort,通道号
//			pFrameDecodeInfo,解码后的音视频数据
//			pFrameInfo,图像和声音信息,请参见FRAME_INFO结构体
//          pDecodeProcessInfo  用于存储解码处理后生成的数据
//			pUser,用户自定义参数
//		 pUser,用户自定义参数
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// 函数: PLAY_ResolutionScale
// 描述: 实际码流分辨率缩放成目的分辨率渲染
// 参数: nPort,通道号
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// 函数: PLAY_SetStereoPerspectiveFovy
// 描述: 设置透镜张开角度，张开的角度越大，显示的图片越小
// 参数: nPort,通道号
//		 fFovy 张开的角度大小（0-180）
//		 nRegionNum,显示区域序号,0~(MAX_DISPLAY_WND-1),如果为0,则将设置的区域显示在主窗口中.
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
...
//------------------------------------------------------------------------
// 函数: PLAY_SetStereoEyeMoveDistance
// 描述: 设置透镜移动的距离
// 参数: nPort,通道号
//		 enMoveDicret 移动的方向
//		 fDistancefDistance,移动的距离.
//		 nRegionNum,显示区域序号,0~(MAX_DISPLAY_WND-1),如果为0,则将设置的区域显示在主窗口中.
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// 函数: PLAY_SetStereoRotate
// 描述: 设置物体旋转的角度
// 参数: nPort,通道号
//		 fXOffset 鼠标在x轴旋转的角度（0-360度）
//		 fYOffset,鼠标在y轴旋转的角度（0-360度）
//		 nRegionNum,显示区域序号,0~(MAX_DISPLAY_WND-1),如果为0,则将设置的区域显示在主窗口中.
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// 函数: PLAY_SetStereoView
// 描述: 调整摄像机的角度
// 参数: nPort,通道号
//		 fXOffset 摄像机以自身作为原点，绕Z轴（即摄像机头顶法线）顺时针旋转的角度增量(以度为单位)
//		 fYOffset 摄像机以自身作为原点，绕X轴顺时针旋转的角度增量(以度为单位)
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
PLAYSDK_API BOOL CALLMETHOD PLAY_SetStereoView(LONG nPort, DWORD nRegionNum, float fXOffset, float fYOffset);

//------------------------------------------------------------------------
// 函数: PLAY_SetStereoViewMode
// 描述: 设置观察的模式
// 参数: nPort,通道号观察的模式，见STEREO_VIEW_MODE
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
PLAYSDK_API BOOL CALLMETHOD PLAY_SetStereoViewMode(LONG nPort, DWORD nRegionNum, STEREO_VIEW_MODE enStereoViewMode);

//------------------------------------------------------------------------
// 函数: PLAY_SetCalibratMode
// 描述: 设置显示的模式
// 参数: nPort,模式，见FISHEYE_CALIBRATMODE
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
...
//------------------------------------------------------------------------
// 函数: PLAY_SetDataCallBack
// 描述: 数据帧回调,回调带大华头的原始数据,外层自己实现解析
// 参数: nPort,通道号
// 返回: BOOL,成功返回TRUE,失败返回FALSE.
//------------------------------------------------------------------------
